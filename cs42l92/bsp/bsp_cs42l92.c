 ...
 * @file bsp_cs42l92.c
 *
 * @brief Implementation of the BSP for the cs42l92 platform.
 *
 * @copyright
...
#define CS42L92_SRC_MUTE            (0x0)
#define CS42L92_SRC_IN1L            (0x10)
#define CS42L92_SRC_IN2L            (0x12)
#define CS42L92_SRC_TONE_GENERATOR1 (0x4)

#define CS42L92_DSP1_CHANNEL1       (0x100)
#define CS42L92_DSP1_CHANNEL2       (0x101)
#define CS42L92_DSP1_CHANNEL3       (0x102)
#define CS42L92_DSP1_CHANNEL4       (0x103)
#define CS42L92_DSP1_CHANNEL5       (0x104)
#define CS42L92_DSP1_CHANNEL6       (0x105)
 ...
 * LOCAL FUNCTIONS
 **********************************************************************************************************************/
void cs42l92_notification_callback(uint32_t event_flags, void* arg);

/***********************************************************************************************************************
 * LOCAL VARIABLES
 **********************************************************************************************************************/
static cs42l92_t cs42l92_driver;
...
static cs42l92_bsp_config_t bsp_config =
{
    ...
    .notification_cb = &cs42l92_notification_callback,
...
static scc_config_t scc_config =
{
    ...
    .cp_config = REGMAP_GET_CP(&cs42l92_driver),
    .fw_info = NULL,
    .state_symbol = CS42L92_SYM_SCC_SCC_STATE,
    .status_symbol = CS42L92_SYM_SCC_SCC_STATUS,
    .error_symbol = CS42L92_SYM_SCC_SCC_ERROR,
    .control_symbol = CS42L92_SYM_SCC_SCC_CONTROL,
    .manageackctrl_symbol = CS42L92_SYM_SCC_SCCMANAGEACKCTRL,
    .enc_format_symbol = CS42L92_SYM_SCC_BUFFER_FORMAT,
    .host_buffer_raw_symbol = CS42L92_SYM_SCC_HOST_BUFFER_RAW,
};
...
{
    .cp = REGMAP_GET_CP(&cs42l92_driver),
    .bufs_config =
    {
        {.base_id = buf1_base, .size_id = buf1_size,      .mem_base = CS42L92_DSP1_XMEM_UNPACKED24_0},
        {.base_id = buf2_base, .size_id = buf1_buf2_size, .mem_base = CS42L92_DSP1_XMEM_UNPACKED24_0},
        {.base_id = buf3_base, .size_id = total_buf_size, .mem_base = CS42L92_DSP1_YMEM_UNPACKED24_0},
    },
    .rb_struct_mem_start_address = CS42L92_DSP1_XMEM_UNPACKED24_0,
    .compr_buf_ptr = NULL,
    .compr_buf_size = 0,
    .buf_symbol = 0,
    .enc_format = COMPR_ENC_FORMAT_PACKED16,
    .bytes_per_reg = CS42L92_DSP_UNPACKED24_BYTES_PER_REG,
};
...
static void bsp_dut_update_i2s_data(void)
{
    ...
    else
    {
        ...
        // CS42L92 only provides a mono stream whereas the I2S is expecting stereo, so duplicate the stream as it
        // is copied in
...
void cs42l92_i2s_callback(uint32_t status, void *arg)
{
    if (bsp_decompressed_data_playing)
    {
...
static uint32_t bsp_scc_init(scc_t *scc)
{
    ...
    ret = regmap_write_fw_control(scc->config.cp_config,
                                  scc->config.fw_info,
                                  CS42L92_SYM_FIRMWARE_INIT_FEATURES,
                                  SCC_FEATURE_SCC);
...
uint32_t bsp_dut_initialize(void)
{
    ...
    cs42l92_config_t codec_config;

    memset(&codec_config, 0, sizeof(cs42l92_config_t));

    // Initialize chip drivers
    ret = cs42l92_initialize(&cs42l92_driver);
    if (ret == CS42L92_STATUS_OK)
    {
        codec_config.bsp_config = bsp_config;

        codec_config.syscfg_regs = cs42l92_syscfg_regs;
        codec_config.syscfg_regs_total = CS42L92_SYSCFG_REGS_TOTAL;

        ret = cs42l92_configure(&cs42l92_driver, &codec_config);
    }
    ...
    // Enable 32kHz clock routing to CS42L92
...
uint32_t bsp_dut_reset(void)
{
    ...
    ret = cs42l92_reset(&cs42l92_driver);
...
uint32_t bsp_dut_boot(void)
{
    ...
    fw_img = cs42l92_fw_img;
    fw_img_end = cs42l92_fw_img + FW_IMG_SIZE(cs42l92_fw_img);

    // Inform the driver that any current firmware is no longer available by passing a NULL
    // fw_info pointer to cs42l92_boot
    ret = cs42l92_boot(&cs42l92_driver, 1, NULL);
    if (ret != CS42L92_STATUS_OK)
    {
        return ret;
    }
    ...
    while (fw_img < fw_img_end)
    {
        ...
        if (ret == FW_IMG_STATUS_DATA_READY)
        {
            ...
            ret = cs42l92_write_block(&cs42l92_driver, boot_state.block.block_addr,
    ...
    ret = cs42l92_boot(&cs42l92_driver, 1, &boot_state.fw_info);
...
{
    uint32_t ret;
    regmap_cp_config_t *cp = REGMAP_GET_CP(&cs42l92_driver);

    regmap_update_reg(cp,
                      CS42L92_DSP_CLOCK1,
                      CS42L92_DSP_CLK_FREQ_MASK,
                      (0x24DD << CS42L92_DSP_CLK_FREQ_SHIFT));
    ret = cs42l92_fll_config(&cs42l92_driver,
                             CS42L92_FLL1,
                             CS42L92_FLL_SRC_ASP1_BCLK,
                             512000,
                             49152000);
    if (ret != CS42L92_STATUS_OK)
    {
    ...
    ret = cs42l92_fll_enable(&cs42l92_driver, CS42L92_FLL1);
    if (ret != CS42L92_STATUS_OK)
    {
        return BSP_STATUS_FAIL;
    }
    ret = cs42l92_fll_wait_for_lock(&cs42l92_driver, CS42L92_FLL1);
    if (ret != CS42L92_STATUS_OK)
    {
        debug_printf("Failed to lock FLL1\n\r");
        return BSP_STATUS_FAIL;
    }
    regmap_write(cp, CS42L92_SAMPLE_RATE1, 0x12);
    regmap_update_reg(cp, CS42L92_SYSTEM_CLOCK1, CS42L92_SYSCLK_EN_MASK, CS42L92_SYSCLK_EN);
    return ret;
}
...
static uint32_t bsp_dut_scc_record(compr_enc_format_t enc_format, uint32_t dsp_core)
{
    uint32_t ret;
    regmap_cp_config_t *cp = REGMAP_GET_CP(&cs42l92_driver);
    cs42l92_dsp_t dsp_info = cs42l92_driver.dsp_info[dsp_core - 1];
    ...
    regmap_update_reg(cp, CS42L92_SAMPLE_RATE1, CS42L92_SAMPLE_RATE_1_MASK, 0x12);
    regmap_update_reg(cp, CS42L92_SAMPLE_RATE2, CS42L92_SAMPLE_RATE_2_MASK, 0x12);

    // Setup sample rates on DSP RXn
    regmap_write(cp, dsp_info.base_addr + CS42L92_DSP_OFF_SAMPLE_RATE_RX1, 0x1);
    regmap_write(cp, dsp_info.base_addr + CS42L92_DSP_OFF_SAMPLE_RATE_RX2, 0x1);
    regmap_write(cp, dsp_info.base_addr + CS42L92_DSP_OFF_SAMPLE_RATE_RX3, 0x1);
    regmap_write(cp, dsp_info.base_addr + CS42L92_DSP_OFF_SAMPLE_RATE_RX4, 0x1);
    regmap_write(cp, dsp_info.base_addr + CS42L92_DSP_OFF_SAMPLE_RATE_RX5, 0x1);
    regmap_write(cp, dsp_info.base_addr + CS42L92_DSP_OFF_SAMPLE_RATE_RX6, 0x1);
    regmap_write(cp, dsp_info.base_addr + CS42L92_DSP_OFF_SAMPLE_RATE_RX7, 0x1);
    regmap_write(cp, dsp_info.base_addr + CS42L92_DSP_OFF_SAMPLE_RATE_RX8, 0x1);
    regmap_write(cp, dsp_info.base_addr + CS42L92_DSP_OFF_SAMPLE_RATE_TX1, 0x0);
    regmap_write(cp, dsp_info.base_addr + CS42L92_DSP_OFF_SAMPLE_RATE_TX2, 0x0);
    regmap_write(cp, dsp_info.base_addr + CS42L92_DSP_OFF_SAMPLE_RATE_TX3, 0x0);
    regmap_write(cp, dsp_info.base_addr + CS42L92_DSP_OFF_SAMPLE_RATE_TX4, 0x0);
    regmap_write(cp, dsp_info.base_addr + CS42L92_DSP_OFF_SAMPLE_RATE_TX5, 0x0);
    regmap_write(cp, dsp_info.base_addr + CS42L92_DSP_OFF_SAMPLE_RATE_TX6, 0x0);
    regmap_write(cp, dsp_info.base_addr + CS42L92_DSP_OFF_SAMPLE_RATE_TX7, 0x0);
    regmap_write(cp, dsp_info.base_addr + CS42L92_DSP_OFF_SAMPLE_RATE_TX8, 0x0);
    regmap_write(cp, CS42L92_DSP1RX2_INPUT1, 0x800012); // IN2L

    // Setup MICBIAS
    regmap_write(cp, CS42L92_MICBIAS_CTRL1, 0x81a5);
    regmap_write(cp, CS42L92_MICBIAS_CTRL5, 0x227);

    // Set up audio input channels
    regmap_update_reg(cp, CS42L92_INPUT2_CONTROL1, 0x50021, 0x50021);
    regmap_write(cp, CS42L92_IN2L_CONTROL1, 0x804);
    regmap_write(cp, CS42L92_INPUT_CONTROL, 0x8); // IN2L_EN
    regmap_write(cp, CS42L92_IN2L_CONTROL2, 0xB00080);
    regmap_write(cp, CS42L92_INPUT_CONTROL3, 0x20000000); // IN_VU

    // Boot and load firmware
    ret = cs42l92_power(&cs42l92_driver, 1 , CS42L92_POWER_MEM_ENA);
    if (ret != CS42L92_STATUS_OK)
    {
    ...
    scc_config.fw_info = cs42l92_driver.dsp_info[scc_config.dsp_core -1].fw_info; //TODO: Should be a getter
    scc_config.cp_config = cp;
    scc_init(&scc, &scc_config, &bsp_scc_init);

    // Set up audio input channels
    regmap_update_reg(cp, CS42L92_DSP1RX2_INPUT1, CS42L92_DSP1RX2_SRC1_MASK, CS42L92_SRC_IN2L);

    // Setup output
    regmap_write(cp, CS42L92_OUTPUT_CONTROL_1, 0x800);
    regmap_update_reg(cp, CS42L92_OUT1L_INPUT1, CS42L92_OUT1L_SRC1_MASK, 0x20);
    regmap_write(cp, CS42L92_OUT1L_VOLUME_1, CS42L92_OUT_VU | 0x64);
    regmap_write(cp, CS42L92_OUTPUT_ENABLE_1, CS42L92_OUT1L_EN_MASK);

    regmap_write(cp, CS42L92_ASP1_ENABLES1, 0x10000);

    ret = cs42l92_power(&cs42l92_driver, 1, CS42L92_POWER_UP);
    if (ret != CS42L92_STATUS_OK)
    {
        return CS42L92_STATUS_FAIL;
    }
...
uint32_t bsp_dut_use_case(uint32_t use_case)
{
    ...
    switch (use_case) {
        ...
        case BSP_USE_CASE_SCC_STOP_RECORDING:
            ...
            // Power down
            ret = cs42l92_power(&cs42l92_driver, 1, CS42L92_POWER_DOWN);
            if (ret != CS42L92_STATUS_OK)
            {
                return BSP_STATUS_FAIL;
            }
            ...
            // Disable inputs
            regmap_write(cp, CS42L92_IN2L_CONTROL1, 0x804);
            regmap_write(cp, CS42L92_INPUT_CONTROL, 0x0); // IN2L_EN=0
            regmap_write(cp, CS42L92_IN2L_CONTROL2, 0x10000000); // IN2L_MUTE
            regmap_write(cp, CS42L92_INPUT_CONTROL3, 0x20000000); // IN_VU
            regmap_update_reg(cp, CS42L92_DSP1RX2_INPUT1, CS42L92_DSP1RX2_SRC1_MASK, CS42L92_SRC_MUTE);

            // MICBIAS
            regmap_write(cp, CS42L92_MICBIAS_CTRL1, 0x81a4);
            regmap_write(cp, CS42L92_MICBIAS_CTRL5, 0x226);

            // Disable DSP memory
            ret = cs42l92_power(&cs42l92_driver, 1 , CS42L92_POWER_MEM_DIS);
            if (ret != CS42L92_STATUS_OK)
            {
                return BSP_STATUS_FAIL;
            }
            ...
            // Disable clock
            regmap_update_reg(cp, CS42L92_SYSTEM_CLOCK1, CS42L92_SYSCLK_EN_MASK, 0);

            // Disable FLLs
            ret = cs42l92_fll_disable(&cs42l92_driver, CS42L92_FLL1);
            if (ret != CS42L92_STATUS_OK)
            {
                return BSP_STATUS_FAIL;
            }
            ret = cs42l92_fll_disable(&cs42l92_driver, CS42L92_FLL2);
            if (ret != CS42L92_STATUS_OK)
            {
                return BSP_STATUS_FAIL;
            }
...
uint32_t bsp_dut_process(void)
{
    ...
    ret = cs42l92_process(&cs42l92_driver);
...
void cs42l92_notification_callback(uint32_t event_flags, void* arg)
{
    if (event_flags & CS42L92_EVENT_FLAG_DSP1_IRQ0)
    {
